-- CLIENT by mstudio45 | original by https://v3rmillion.net/member.php?action=profile&uid=1802731 
-- https://v3rmillion.net/showthread.php?tid=1200475

local AZUMA_START_TIME = os.time()
...
local s,e = pcall(function()
	gui_data = game:HttpGet(("https://raw.githubusercontent.com/mstudio45/AZUMA_CLIENT/main/gui_data.json"), true)
	gui_data = game:GetService("HttpService"):JSONDecode(gui_data)
end)
if e then
	oldwarnmessage("AZUMA CLIENT v"..currentver, "Failed to get script data.", 10)
end

if AZUMALOADED == true then warnmessage("AZUMA CLIENT v"..currentver, "GUI already loaded!", "", 10) return end
if game.PlaceId ~= 6839171747 and game.PlaceId == 6516141723 then 
	--warnmessage("AZUMA CLIENT v"..currentver, "You need to join a game to run this script.", 10) 
	confirmnotification("AZUMA CLIENT v"..currentver, "Do you want to join a game?", 15, function(state)
		if state == true then
			task.spawn(function()
				loadstring(game:HttpGet(("https://raw.githubusercontent.com/mstudio45/AZUMA_CLIENT/main/joinsolo.lua"), true))()
			end)
		end
	end)
	return
...
if game.PlaceId ~= 6839171747 and game.PlaceId ~= 6516141723 then 
	oldwarnmessage("AZUMA CLIENT v"..currentver, "You need to join DOORS to run this script.", 10) 
	return
end
if gui_data ~= nil then
	if currentver ~= gui_data.ver or gui_data.ver ~= currentver then
		warnmessage("AZUMA CLIENT v"..currentver, "You are using an outdated version of this script", "Loading latest version.", 10) 
...
pcall(function() getgenv().AZUMALOADED = true end)
normalmessage("AZUMA CLIENT v"..currentver, "Loading script...", "", 2)
if gui_data ~= nil then
...
function JoinDiscord(InviteCodee)
	...
	if not RequestFunction then
		oldwarnmessage("AZUMA CLIENT v"..currentver, "Your executor does not support http requests.", 5)
		return
	end
	for i = 6453, 6464 do
...
function PathModule.new(char, goal, agentParameters, jumpingAllowed, offset)
	...
	if Success and Path.Status == Enum.PathStatus.Success then 
		...
		for i, v in pairs(waypoints) do
			if AZUMALOADED == false or not v then return end
...
--local library = loadstring(game:HttpGet(--[['https://pastebin.com/raw/vPWzQEC8'--]]"https://raw.githubusercontent.com/mstudio45/AZUMA_CLIENT/main/library.lua"))()
...
local GUIWindow = Library:CreateWindow({
	Name = "AZUMA CLIENT v".. currentver,
	Themeable = false
})
...
if isfolder and makefolder and listfiles and writefile and delfile then
	local AZUMA_CLIENT_FOLDER_NAME = "AZUMA_CLIENT"
	local function checkdir() if not isfolder(AZUMA_CLIENT_FOLDER_NAME) then makefolder(AZUMA_CLIENT_FOLDER_NAME) end end
	checkdir()

	local filetablelist = {}	
	function reloadList(ConfigDropdowne)
		filetablelist = {}
		for _,v in pairs(listfiles(AZUMA_CLIENT_FOLDER_NAME)) do
			local filename = v
			filename = string.gsub(filename, "AZUMA_CLIENT", "")
	...
	function saveConfig(name, replace)
		...
		local fileexists = isfile(AZUMA_CLIENT_FOLDER_NAME.."/"..name..".json")
		if replace == false then
			oldnormalmessage("CONFIGS", "Trying to save config called '"..name.."'.", 5)
			if not isfile(AZUMA_CLIENT_FOLDER_NAME.."/"..name..".json") then
				local s,e
				repeat task.wait()
					s,e = pcall(function()
						local jsonflags = game.HttpService:JSONEncode(flags)
						writefile(AZUMA_CLIENT_FOLDER_NAME.."/"..name..".json", jsonflags)
					end)
				until not e and s
	...
	function loadConfig(name, notifs)
		...
		if isfile(AZUMA_CLIENT_FOLDER_NAME.."/"..name..".json") then
			local jsonecoded = readfile(AZUMA_CLIENT_FOLDER_NAME.."/"..name..".json")
	...
	function deleteConfig(name, notifs)
		...
		if isfile(AZUMA_CLIENT_FOLDER_NAME.."/"..name..".json") then
			...
			repeat task.wait()
				s,e = pcall(function()
					delfile(AZUMA_CLIENT_FOLDER_NAME.."/"..name..".json")
				end)
			until not e and s
	...
	CONFIG_AUTO:AddButton({ 
		...
		Callback = function() 
			...
			repeat task.wait()
				s,e = pcall(function()
					writefile(AZUMA_CLIENT_FOLDER_NAME.."/autostart.txt", ConfigDropdown:Get())
				end)
			until not e and s
	...
	CONFIG_AUTO:AddButton({ 
		...
		Callback = function() 
			...
			repeat task.wait()
				s,e = pcall(function()
					delfile(AZUMA_CLIENT_FOLDER_NAME.."/autostart.txt")
				end)
			until not e and s
	...
	task.spawn(function()
		...
		if isfile(AZUMA_CLIENT_FOLDER_NAME.."/autostart.txt") then
			local autostart_name = readfile(AZUMA_CLIENT_FOLDER_NAME.."/autostart.txt")
			if isfile(AZUMA_CLIENT_FOLDER_NAME.."/"..autostart_name..".json") then
...
newnotificationsettings:AddButton({
	...
	Callback = function()
		normalmessage("AZUMA CLIENT v"..currentver, "This is a test!", "Hey.", 5, nil, "TEST NOTIFICATION")
	end
})
...
oldnotificationsettings:AddButton({
	Name = "Test Notification",
	Callback = function()
		oldnormalmessage("AZUMA CLIENT v"..currentver.." [TEST]", "This is a test!", 5)
	end
})
...
window_credits:AddLabel({ Name = "CLIENT by: mstudio45" })
...
if fireproximityprompt then
	...
	warnmessage("AZUMA CLIENT v"..currentver, "You need to have fireproximityprompt function for 'instant use'.", 7)
end
...
window_player:AddButton({
	...
	Callback = function()
		confirmnotification("AZUMA CLIENT v"..currentver, "Are you sure to reset your character?", 15, function(state)
			if state == true then
...
if hookmetamethod and newcclosure and getnamecallmethod then
	...
	warnmessage("AZUMA CLIENT v"..currentver, "You need to have hookmetamethod and newcclosure and getnamecallmethod functions for 'always win heartbeat'.", 7)
end
...
if fireproximityprompt then
	...
	oldwarnmessage("AZUMA CLIENT v"..currentver, "You need to have fireproximityprompt function for 'skip room'.", 7)	
end
...
if fireproximityprompt then
	...
	oldwarnmessage("AZUMA CLIENT v"..currentver, "You need to have fireproximityprompt function for 'loot aura'.", 7)
end
...
if fireproximityprompt then
	...
	oldwarnmessage("AZUMA CLIENT v"..currentver, "You need to have fireproximityprompt function for 'book aura'.", 7)
end
...
if fireproximityprompt then
	...
	oldwarnmessage("AZUMA CLIENT v"..currentver, "You need to have fireproximityprompt function for 'lever aura'.", 7)
end
	...
	LatestRoom:GetPropertyChangedSignal("Value"):Connect(function()
		if flags.autorooms == true then
			if LatestRoom.Value ~= 1000 then
				...
				plr.DevComputerMovementMode = Enum.DevComputerMovementMode.KeyboardMouse
				autoa1000:Set(false)
				normalmessage("AUTO A-1000", "Finished walking to A-1000!", "Thanks for using AZUMA CLIENT Auto A-1000.", 10)
			end
		else
			plr.DevComputerMovementMode = Enum.DevComputerMovementMode.KeyboardMouse
		end
...
function closegui()
	...
	pcall(function() getgenv().AZUMALOADED = false;AZUMALOADED = false end)

	task.wait(.1)
	normalmessage("AZUMA CLIENT v"..currentver, "GUI closed!")
end
...
window_guisettings:AddButton({
	...
	Callback = function()
		task.spawn(function()
			repeat task.wait(.1) until AZUMALOADED == false or AZUMALOADED == nil
			loadstring(game:HttpGet((gui_data.loadstring),true))()
		end)
		Library.unload()
	end
...
if game:GetService("UserInputService").TouchEnabled and not game:GetService("UserInputService").KeyboardEnabled and not game:GetService("UserInputService").MouseEnabled then
	...
	local MobileToggle = {};MobileToggle["1"] = Instance.new("ScreenGui", game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui"));MobileToggle["1"]["ZIndexBehavior"] = Enum.ZIndexBehavior.Sibling;MobileToggle["ToggleButton"] = Instance.new("TextButton", MobileToggle["1"]);MobileToggle["ToggleButton"]["TextWrapped"] = true;MobileToggle["ToggleButton"]["BorderSizePixel"] = 0;MobileToggle["ToggleButton"]["TextScaled"] = true;MobileToggle["ToggleButton"]["BackgroundColor3"] = Color3.fromRGB(255, 255, 255);MobileToggle["ToggleButton"]["TextColor3"] = Color3.fromRGB(0, 0, 0);MobileToggle["ToggleButton"]["Size"] = UDim2.new(0.049780379980802536, 0, 0.08735332638025284, 0);MobileToggle["ToggleButton"]["Name"] = [[Tbtn]];MobileToggle["ToggleButton"]["Text"] = [[Toggle AZUMA CLIENT]];MobileToggle["ToggleButton"]["Font"] = Enum.Font.FredokaOne;MobileToggle["ToggleButton"]["Position"] = UDim2.new(0.872797429561615, 0, 0.10329286754131317, 0);
...
scriptLoaded = true
normalmessage(
	"AZUMA CLIENT v"..currentver, "Script loaded!", 
	"Took "..(os.time() - AZUMA_START_TIME).."ms.", 
	5